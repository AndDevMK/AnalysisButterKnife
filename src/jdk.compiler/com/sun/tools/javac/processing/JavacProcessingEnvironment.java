...
public class JavacProcessingEnvironment implements ProcessingEnvironment, Closeable {
    ...
    protected JavacProcessingEnvironment(Context context) {
        ...
        // 获取Processor的loader，也就是加载器
        initProcessorLoader();
    }

    public void setProcessors(Iterable<? extends Processor> processors) {  // null
    ...
    private void initProcessorIterator(Iterable<? extends Processor> processors) {   // null
        ...
        Iterator<? extends Processor> compoundIterator =
                ...
                // 所有注解处理器最终包装进DiscoveredProcessors类，方便之后迭代，这里获取迭代器对象时会调用了process的init
    ...
    private void discoverAndRunProcs(Set<TypeElement> annotationsPresent,  
        ...
        // 创建一个注解Map集合，装载没有匹配上的注解元素（默认当作全是没匹配上）
        ...
        // 创建一个set集合，用来装载所有注解信息，放入到RoundEnvironment中
        ...
        // 创建一个RoundEnvironment对象，也就是注解处理器process方法的一个参数
        RoundEnvironment renv = new JavacRoundEnvironment(false,
                                                          false,
                                                          rootElements,
                                                          JavacProcessingEnvironment.this);
        // 遍历所有没有匹配上的注解元素（默认当作全是没匹配上）
        while(unmatchedAnnotations.size() > 0 && psi.hasNext() ) {
            ProcessorState ps = psi.next();
            // 创建一个set集合，用来装载匹配上的注解名称
            ...
            if (matchedNames.size() > 0 || ps.contributed) {
                ...
                // 如果process方法返回true，则没有匹配上的set集合移除该注解信息，也就是由当前处理器处理，其它处理获取不到注解信息，返回false由接下来的处理器处理
                if (processingResult) {
    ...
    class Round {
        ...
        // lastRound第一次执行为false，最后一次执行为true
    ...
    public boolean doProcessing(List<JCCompilationUnit> roots,
        ...
        do {
            ...
            moreToDo = moreToDo();  // 生成新文件，继续循环，导致run会执行第二次
            ...
            // 第二轮循环之前：创建新的round对象
            // 如果生成的文件中又使用了该注解，那么还要接着处理该注解，也就是处理套娃的情况
            round = round.next(
        ...
        } while (moreToDo && !errorStatus);   // 如果有文件生成，且没有出错，执行下一次循环
